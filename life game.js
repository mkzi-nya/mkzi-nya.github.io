(function () {
  ...
  function initGrid() {
    ...
    nextGrid = Array.from({ length: gridHeight }, () =>
      Array(gridWidth).fill(0)
    );
    inactiveTime = Array.from({ length: gridHeight }, () =>
      Array(gridWidth).fill(0)
    );
    cellAge = Array.from({ length: gridHeight }, () =>
      Array(gridWidth).fill(0)
    );
  ...
  function updateSimulation() {
    // 用于保存本次更新后细胞的年龄信息
    let newAge = Array.from({ length: gridHeight }, () =>
      Array(gridWidth).fill(0)
    );
  ...
  function drawSimulation() {
    ...
    for (let i = 0; i < gridHeight; i++) {
      for (let j = 0; j < gridWidth; j++) {
        if (grid[i][j] && cellAge[i][j] >= HIDE_ITERATIONS) {
          // **仅绘制存活超过 HIDE_ITERATIONS 的细胞**
